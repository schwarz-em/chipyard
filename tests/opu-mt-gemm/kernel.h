...
void i8_loop_k_general(int32_t* c, int8_t* at, int8_t* b, size_t lda, size_t N, size_t K, size_t ml, size_t vl) {
  asm volatile("vle32.v v0, (%0)" : : "r"(c));
  OPMVINBCAST(m0, v0); // move v0 into row r of m1
  for (size_t k = 0; k < K; k++) {
      asm volatile("vsetvli zero, %0, e8, m1, ta, ma" : : "r"(ml));
      asm volatile("vle8.v v5, (%0)" : : "r"(&at[k*lda]));
...
void i8_lm2_loop_k(int32_t* c, int8_t* at, int8_t* b, size_t lda, size_t N, size_t K, size_t ml) {
  ...
  size_t k = 0;
  while (k + 2 <= K) {
      asm volatile("vle8.v v16, (%0)" : : "r"(&at[k*lda]));
      asm volatile("vle8.v v18, (%0)" : : "r"(&b[k*N]));
      VOPACC(m0, v18, v16);
      VOPACC(m2, v18, v17);
      k++;
      asm volatile("vle8.v v20, (%0)" : : "r"(&at[k*lda]));
...
void i8_mm_bme_2x2(int32_t* c_bias, int32_t* c_out, int8_t* at, int8_t* b, size_t M, size_t N, size_t K, size_t lda) {
  size_t mlmax;
  ...
  while (i + 2*mlmax <= M) {
    ...
    while (j + 2*mlmax <= N) {
      i8_lm2_loop_k(&c_bias[j], &at[i], &b[j], lda, N, K, mlmax);
      i32_lm2_store_c(&c_out[(i*N)+j], mlmax, N);
      j += 2*mlmax;
    }
    while (j < N) {
      asm volatile("vsetvli %0, %1, e32, m4, ta, ma" : "=r"(vl) : "r"(N - j));
      i8_loop_k_general(&c_bias[j], &at[i], &b[j], lda, N, K, mlmax, vl);
      i32_store_c(&c_out[(i*N)+j], mlmax, vl, N);
      i8_loop_k_general(&c_bias[j], &at[i+mlmax], &b[j], lda, N, K, mlmax, vl);
  ...
  while (i < M) {
    ...
    for (size_t j = 0; j < N;) {
      asm volatile("vsetvli %0, %1, e32, m4, ta, ma" : "=r"(vl) : "r"(N - j));
      i8_loop_k_general(&c_bias[j], &at[i], &b[j], lda, N, K, ml, vl);
