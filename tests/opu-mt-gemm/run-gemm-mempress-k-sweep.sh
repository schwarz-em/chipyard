#!/usr/bin/env bash
#
# Sweep the K dimension of the multichip mempress GEMM (opu-multichip-mempress-gemm)
# over powers of two, running each on UcieMempressDualIrisConfig in
...
#
#   make run-binary CONFIG=UcieMempressDualIrisConfig USE_CHISEL7=1 \
#       BINARY=../../tests/build/opu-multichip-mempress-gemm.riscv LOADMEM=1 \
...
CONFIG="${CONFIG:-UcieMempressDualIrisConfig}"
...
for K in $KS; do
  ...
  echo "================ K=$K : running ================"
  log="$GEMM_DIR/k-sweep-mempress-$K.run.log"
  ( cd "$SIM_DIR" && make run-binary \
      CONFIG="$CONFIG" \
      USE_CHISEL7=1 \
      BINARY=../../tests/build/opu-multichip-mempress-gemm.riscv \
      LOADMEM=1 \
      EXTRA_SIM_FLAGS="+chip_id0=0x00004080:0x00000001 +chip_id1=0x00004080:0x00000002" \
