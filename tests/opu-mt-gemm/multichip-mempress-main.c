...
// mempress RoCC opcode (OpcodeSet.custom2) and funct codes (see ctrl.scala).
#define MEMPRESS_OPCODE                2
#define FUNCT_PARSE_GLOBAL_STREAM_INFO 1
#define FUNCT_PARSE_LOCAL_STREAM_INFO  2
#define FUNCT_GET_CYCLE                3

int8_t *b_c0 = (int8_t*)TCM_BASE_C0;

// Aligned to the 32-byte beat so each transfer is naturally aligned; mempress
// also handles arbitrary alignment internally.
...
// Issue a mempress memcopy (single stream / single copy) and block until the
// accelerator reports completion (GET_CYCLE unblocks only once every write has
// been acknowledged, i.e. the data has landed at the destination chip).
static inline uint64_t mempress_memcpy(uint64_t dst, uint64_t src, uint64_t len) {
  unsigned long acc_cycles;
  uint64_t stream_cnt = 1;
  asm volatile ("fence");
  ROCC_INSTRUCTION_SS(MEMPRESS_OPCODE, stream_cnt, len, FUNCT_PARSE_GLOBAL_STREAM_INFO);
  ROCC_INSTRUCTION_SS(MEMPRESS_OPCODE, dst, src, FUNCT_PARSE_LOCAL_STREAM_INFO);
  ROCC_INSTRUCTION_D(MEMPRESS_OPCODE, acc_cycles, FUNCT_GET_CYCLE);
...
void __main(void) {
  ...
  if (chip_id != 1) {
    ...
    // link via the mempress memcopy engine, using the offchip-alias trick.
    ...
    // Make the GEMM's c_opu stores visible before the accelerator reads them.
    __sync_synchronize();

    size_t memcpy_start = read_csr(mcycle);
    mempress_memcpy((uint64_t)remote_c_slice,
                    (uint64_t)(c_opu + m_start * N_DIM), slice_bytes);
    size_t memcpy_end = read_csr(mcycle);

    // Order the C write before the done-flag write.
    __sync_synchronize();
    printf("Chip %lu: chiplet mempress memcpy %lu cycles\n", chip_id, memcpy_end - memcpy_start);
    ...
    *remote_flag = 1;
    ...
    // every chip to finish, so a worker must complete rather than park.
